...
if ($logFiles)
{
    ...
    foreach($log in $logFiles)
    {
        ...
        if ($dpNameList -imatch $dpToCheck)
        {
            ...
            Write-CMTraceLog -Message "$dpToCheck installed successful"

            # Get list of assigned content to the DP
            $dpNALPath = ($dpNameList -imatch $dpToCheck) -replace '\\', '\\' # we ned to add some "\" for the WMI query to work
            $wmiQuery = "SELECT * FROM SMS_DPContentInfo WHERE NALPath = '$dpNALPath'"
            ...
            if ($smsDPStatusInfo.NumberInstalled -eq $assignedContentCount)
            {
                ...
                If ($sendMail){Send-MailMessage -From $mailFrom -SmtpServer $mailserver -Subject "$dpToCheck DP install successful" -To $mailTo}
            }
            else
            {
                Write-CMTraceLog -Message "$dpToCheck NumberInstalled $($smsDPStatusInfo.NumberInstalled), NumberErrors $($smsDPStatusInfo.NumberErrors), NumberInProgress $($smsDPStatusInfo.NumberInProgress), NumberUnknown $($smsDPStatusInfo.NumberUnknown)"
                If ($smsDPStatusInfo.NumberErrors -gt 0)
                {
                    Write-CMTraceLog -Message "$dpToCheck content in error state. Will move file to failed folder."   
                    If ($moveFiles){Move-Item -Path ($log.FullName) -Destination $failureFolder -Force}
                    If ($sendMail){Send-MailMessage -From $mailFrom -SmtpServer $mailserver -Subject "$dpToCheck DP content in error state" -To $mailTo}
                }

                if ($moveFileToFailure)
                {
                    Write-CMTraceLog -Message "$dpToCheck Content still not there yet. Will move file to failure" -Type Warning
                    If ($moveFiles){Move-Item -Path ($log.FullName) -Destination $failureFolder -Force}
                    If ($sendMail){Send-MailMessage -From $mailFrom -SmtpServer $mailserver -Subject "$dpToCheck DP content still not there. Max timeout reached" -To $mailTo}
                }
                else
                {
                    Write-CMTraceLog -Message "$dpToCheck Content still not there yet. Will be checked again next time the script runs"
                }
        ...
        else
        {
            if ($moveFileToFailure)
            {
                Write-CMTraceLog -Message "$dpToCheck No install success found. Will move file to failure" -Type Warning
                If ($moveFiles){Move-Item -Path ($log.FullName) -Destination $failureFolder -Force}
                If ($sendMail){Send-MailMessage -From $mailFrom -SmtpServer $mailserver -Subject "$dpToCheck DP install not successful. Max timeout reached" -To $mailTo}
            }
            else
            {
