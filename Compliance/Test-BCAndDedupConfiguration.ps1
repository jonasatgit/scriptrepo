...
.PARAMETER ConnectedCacheFolderName
    ...
    Default is 'DOINC-E77D08D0-5FEA-4315-8C95-10D359D59294'
...
{    
    ...
    if ($Service.BranchCacheIsEnabled -ine "True") 
    {
        if ($Remediate) 
        {
    ...
    if ($Service.BranchCacheServiceStartType -ine "Automatic") 
    {
        if ($Remediate) 
        {
    ...
    if ($Service.BranchCacheServiceStatus -ine "Running") 
    {
        if ($Remediate) 
        {
